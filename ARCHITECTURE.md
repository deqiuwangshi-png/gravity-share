...
| 三点菜单视口避让（2026-09-03） | ✅ | 问题：弹层写死 `absolute right-0 top-[calc(100%+4px)]`，在页面底部/窄屏点开时超出视口被裁（已排查祖先无 overflow 裁剪，纯视口边界溢出）。**方案 B（用户拍板，仅 PostMenu，UserMenu 暂不改）**：引入 `@floating-ui/react` ^0.27.20，`useFloating({ placement:"bottom-end", strategy:"absolute", middleware:[offset(4), flip({padding:8}), shift({padding:8}), size({padding:8, 限高 140 下限})], whileElementsMounted:autoUpdate })`；触发器挂 `refs.setReference`、弹层挂 `refs.setFloating` + `floatingStyles`。**关键取舍：不挂 FloatingPortal** —— 弹层仍是按钮容器 DOM 子节点，点击外部关闭（`ref.current.contains`）与 Esc 关闭逻辑零改动；`strategy:"absolute"` 与 React 19/Next 16 兼容（未用 CSS Anchor Positioning，浏览器支持不足）。弹层加 `overflow-y-auto` 配合 size 限高；新增 1 个运行时依赖（~10KB gz，传递含 @floating-ui/dom + react-dom），是自研与 AGENTS.md「禁止自研 Dropdown/Popover 通用能力」冲突下的生态方案。**踩坑（必守）：`refs` 不能直接 `refs.setReference/ref={refs.setFloating}` 传给 JSX ref** —— React Compiler 对 JSX ref 属性有「传入值形如 useRef 返回值」的类型方程，`refs.setX` 属性读取被反向推断成渲染期读取 ref 值，报 `react-hooks/refs`（已知误报 facebook/react#34775）。正确写法：`const { refs: floatingRefs, floatingStyles } = useFloating(...); const { setReference, setFloating } = floatingRefs;` 再 `ref={setReference}` / `ref={setFloating}` |
