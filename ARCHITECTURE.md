...
| 社交分享卡片 | `app/opengraph-image.tsx` + `app/twitter-image.tsx`（两者同源：twitter 直接 re-export og，1200×630，next/og build 期静态生成）+ `app/fonts/og-sc-bold.otf`（思源黑体 Bold OFL 子集，~10KB） | Next.js 文件约定自动注入 og:image / twitter:image（og 覆盖微信·微博·飞书，twitter 覆盖 X）；**文案改动须同步 `app/fonts/og-chars.txt` 重新生成子集**，否则新字缺字形 |
