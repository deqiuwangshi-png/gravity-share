...
| CSS 迁移 P-通知：通知预览抽屉 Tailwind 化（2026-09-02） | ✅ | (app) 区通知预览抽屉迁移（用户拍板：按方案一次全迁）。notification.css(216) 单文件单组件（notification-drawer.tsx 双 export Trigger/Drawer，无跨页牵连）全量原子类化后删除、layout import 清理（17→16，app 区 14→13）；15 个 notify-* 类逐一映射（铃铛/红点/遮罩/抽屉/头部/列表/条目/空态 + open 态，令牌全覆盖）；三处实现迁移：① open 状态机父选择器（.notify-layer.open .overlay/.drawer 控 opacity/translateX/pointer-events）改 JSX 条件类驱动（组件常驻挂载 + open 为 prop，遮罩 240ms / 抽屉 300ms cubic-bezier(0.32,0.72,0.35,1) 动画逐字保留）；② .unread::before 伪元素圆点改条件真实 span（Tailwind 无伪元素对应）；③ trigger 展开态原 [aria-expanded]（特异性 0,1,1）< :hover（0,2,0）打开悬停仍显 hover 色——Tailwind aria-expanded: 变体生成序无法保证该层级 → open prop 条件 base 类 + 无条件 hover 类，层叠与原 CSS 逐位一致；抽屉 left/top 原子类任意值 `left-[var(--app-sidebar-w)]`/`top-[var(--app-topbar-h)]` 继续引用 decor ① 壳变量宿主（≤800 抽屉全宽联动，宿主变量保留仅注释更新）；decor.css 追加 ⑧ 收容 2 条非令牌 rgba（notify-overlay 遮罩 rgba(17,24,22,.28)、notify-drawer 左侧投影 rgba(17,24,22,.06)，宿主类名 JSX 保留）。零新依赖；待用户本地 pnpm check + build + 视觉冒烟 |
| CSS 迁移 P-菜单·toast·富文本（2026-09-02） | ✅ | (app) 区用户下拉菜单/轻量提示/富文本样式/渲染端排版/三点菜单五件迁移（用户拍板：同批一次做掉 + 先分析重复复用 + 富文本双排版层合并 + h3 统一 16px + --on-error 令牌收口）。user-menu.css(129)+comment-menu.css(79)+toast.css(38)+rich-editor.css(318) 四文件迁 Tailwind 后删除、layout import 清理（**16→12**，app 区 13→9）；组件层：UserMenu 原子类化（抽 itemClass/iconClass 局部常量、guest 登录/注册入口同款、入场动画任意值引用 decor keyframes；死规则 .user-menu-trigger strong 删除——纯头像菜单无昵称）/ Toast（danger bg-error text-on-error、neutral bg-foreground text-surface、toast-cycle 2.2s 动画引用 decor）/ PostMenu（三点菜单弹层样式随组件并入，抽 MENU_ITEM_CLASS/MENU_ITEM_DANGER_CLASS，危险项 hover bg-error text-on-error，confirm 区 12px）/ RichEditor（toolbar 钮/图集条原子类化、placeholder 绝对定位、图集条宿主类保留供 decor 挂浮层）；**富文本双排版层合并去重**（本批核心）：编辑端 .rich-editor-content 与渲染端 .rich-content 13 组规则逐条比对仅 h3 上边距 2px 历史差（18/16）+ img 大图/小图有意差异 → 合并为并列选择器共享排版层一处维护两处消费，h3 统一 16px；globals 补 --on-error:#fff + @theme --color-on-error（error 红底白字第 3 次出现全局收口，decor ⑦ close-confirm-discard:hover 白字回改走令牌）；decor.css 追加 ⑨ 收容 3 条（@keyframes user-menu-in / toast-cycle + .rich-gallery-status/.rich-gallery-actions 黑底 rgba）。零新依赖；待用户本地 pnpm check + build + 视觉冒烟 |
