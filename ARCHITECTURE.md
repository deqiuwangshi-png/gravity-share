...
| 后端 / 数据库 / API 层 | ⛔ 冻结 | 不建 `lib/api`、不写 fetch、不加依赖；**路线已定：Supabase 托管 BaaS**（见 docs/design/auth-architecture.md v2） |
| 认证与登录守卫 | ⏸ 推迟（路线已定） | 采用 **Supabase Auth**（托管）；方案已定稿，落地待阶段 1 启动；当前无守卫 |
| 数据访问层异步化 | ⛔ 冻结 | 同步 mock 常量保持现状；阶段 1 由 Supabase 查询替代（保留前端形态） |
