...
supabase/migrations/   001-029（users → 内容 → 互动 → 存储 → 公开读 → 分类 → views →
...
| 发布 | 单一发布弹窗（2026-08-27 三入口合并；2026-08-31 移除可选标注，新帖统一普通分享；2026-09-02 标题改必填）：标题（必填）+ 轻量富文本正文（B/斜体/无序列表/图片，链接靠输入/粘贴自动识别）+ 图集（≤9 张第 1 张作封面）+ 分类（12 选 1 默认「其他」），写库后列表实时刷新 |
