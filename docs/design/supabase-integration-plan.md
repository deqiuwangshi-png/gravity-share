...
> 版本：v2.0 · 日期：2026-08-21 · 状态：**1a 认证基座 ✅ 已落地**
> 前置：auth-architecture.md v2（Supabase 路线定稿）· auth-system-design.md（五层设计）
> 原则：小步快跑——**1a 认证基座（已落地）→ 1b 数据上库（另行排期）**
...
| 分期 | 范围 | 状态 |
|---|---|---|
| **1a · 认证基座** | 依赖 + env + 双客户端 + proxy 守卫 + 登录/注册接 SDK + 我的账户接 session + 个人主页接 profile | **✅ 已落地（2026-08-21）** |
