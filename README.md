...
- **付费流 = 确定性分发**：承诺位置、时间、数量、验证四要素，不做算法个性化，不绑架自然流量。
...
引力不自己搭后端。认证、数据库、行级安全、对象存储、触发器，全部由 **Supabase BaaS** 承担；支付收单、税务、发票、拒付、退款，全部由 **Waffo Pancake（Merchant of Record）** 托管。代码里只有三样东西：**页面组装、组件交互、业务规则**。这让任何人都能在 10 秒内看懂项目结构，也让维护成本保持在最低。
...
| 支付 | Waffo Pancake（MoR 托管：订阅 USD / 投流 CNY，零自研收单） |
...
| 应用 | `/home` `/square(/[id])` `/categories(/[slug])` `/tag/[tag]` `/profile(/[id])` `/promo`（订阅）`/boost`（投流） | 需登录（proxy.ts 守卫） |
| 网关 | `/go`（外链出站统一网关） | 公开 |
...
- **付费流承诺公开** —— 位置、时间、数量、验证四要素写死，不搞暗箱竞价。

这套方案的核心理念是：**把分发权交还给规则，而不是交给黑盒。** 规模上来之前，不需要算法；规模上来之后，算法也只作用于免费流，付费承诺永远不动。

### 工程：为什么极简 + 托管

- **托管而非自建**：认证、数据库、存储、支付全部交给成熟基础设施（Supabase / Waffo），项目只保留业务逻辑。MVP 阶段，验证理念比堆技术重要得多；
